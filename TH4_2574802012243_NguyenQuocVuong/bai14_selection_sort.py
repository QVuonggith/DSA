...
Cài đặt selection sort trên danh sách liên kết đơn: mỗi vòng tìm nút
giá trị nhỏ nhất ở phần chưa sắp xếp và nối vào danh sách kết quả.
...
def selection_sort_linked_list(head):
    ...
    while head:
        # Tìm node có giá trị nhỏ nhất trong phần chưa sắp xếp
        min_node = head
        min_prev = None
        ...
        # Gỡ min_node ra khỏi danh sách gốc
        if min_node is head:
            head = head.next
        else:
        ...
        # Nối min_node vào danh sách kết quả
...
if __name__ == "__main__":
    ...
    print(linked_to_list(sorted_head))  # [1, 2, 3]
